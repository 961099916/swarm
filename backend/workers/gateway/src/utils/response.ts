...
export class ResponseBuilder {
  ...
  public static error(errorMsg: string, traceId: string, status = 400): Response {
    const code = status === 401 ? 1010 : status === 403 ? 1020 : status === 404 ? 1030 : status === 409 ? 1040 : 1000;
  ...
  /** @deprecated 使用 ApiRes.conflict() */
  public static conflict(errorMsg: string, traceId: string): Response {
    return ApiRes.conflict(errorMsg, traceId);
  }
}
