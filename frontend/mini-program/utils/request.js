  ...
  return new Promise((resolve, reject) => {
    wx.request({
      ...
      success: (res) => {
        ...
        // 1020: 权限不足
        if (bizCode === 1020) {
          showErrorModal('账号异常提示', errMsg || '您无权执行此操作，账号可能已被限制', traceId);
        ...
        if (statusCode === 421) {
          showErrorModal('服务路由异常', 'API 网关路由配置异常，请联系管理员检查 Cloudflare Workers 部署状态', traceId);
        ...
        // 其他错误
        showErrorModal('服务处理失败', errMsg || `请求未成功 (状态码: ${statusCode})`, traceId);
        reject(new Error(errMsg || `服务器故障 (状态码: ${statusCode})`));
      },
      fail: (err) => {
        showErrorModal('网络连接失败', '网络连接失败，请检查网络设置或稍后重试。', traceId);
        reject(err);
      },
    });
  });
...
/**
 * 展示带有 TraceID 并支持一键复制的友好容灾 Modal
 * @param {string} title 
 * @param {string} content 
 * @param {string} traceId 
 */
function showErrorModal(title, content, traceId) {
  wx.showModal({
    title: title || '系统提示',
    ...
    cancelText: '关闭',
    success: (res) => {
      if (res.confirm && traceId) {
        wx.setClipboardData({
          data: traceId,
          success: () => {
            wx.showToast({ title: '复制已完成', icon: 'success' });
          }
        });
      }
    }
